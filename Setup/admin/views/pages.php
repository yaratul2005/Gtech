            ...
            <?php foreach ($pages as $p): ?>
                <div class="flex-between" id="page-row-<?php echo htmlspecialchars($p['slug']); ?>" 
                    ...
                    <div style="display: flex; gap: 8px;">
                        <button class="btn btn-secondary btn-sm edit-page-btn" 
...
<div id="workspace-overlay" class="workspace-panel" style="display: none;">
    <div class="workspace-header">
        <h3 id="workspace-title">Create Page</h3>
    ...
    <div class="workspace-body">
        <div class="card" style="padding: 0px; border-radius: 16px; border: none; background: transparent;">
            <div class="form-alert form-alert-success" id="add-page-success" style="font-size: 0.82rem; padding: 12px; margin-bottom: 20px;"></div>
            <div class="form-alert form-alert-error" id="add-page-error" style="font-size: 0.82rem; padding: 12px; margin-bottom: 20px;"></div>

            <form id="page-add-form" action="/admin/pages/create" method="POST">
                <input type="hidden" name="csrf_token" value="<?php echo csrf_token(); ?>">
                <input type="hidden" name="original_slug" id="page-original-slug" value="">
                ...
                <div class="form-group" style="margin-bottom: 25px;">
                    ...
                    <textarea name="content" id="page-editor" class="form-input" style="min-height: 250px; font-family: monospace; font-size: 0.85rem;" placeholder="Write raw HTML content..."></textarea>
                </div>

                <div style="display: flex; gap: 10px; margin-top: 20px;">
                    <button type="submit" class="btn btn-primary" id="add-page-submit" style="min-width: 150px;">Create Page</button>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const pageOriginalSlug = document.getElementById('page-original-slug');
    ...
    openAddWorkspace.addEventListener('click', () => {
        addForm.reset();
        addForm.action = '/admin/pages/create';
        pageOriginalSlug.value = '';
        formHeading.textContent = 'Create Page';
    ...
    addForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                throw new Error(data.message || 'Saving page failed.');
            }
        } catch (err) {
            errorAlert.textContent = err.message;
            errorAlert.style.display = 'block';
            submitBtn.disabled = false;
            submitBtn.textContent = addForm.action.includes('update') ? 'Update Page' : 'Create Page';
        }
    });
    ...
    document.querySelectorAll('.edit-page-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            pageOriginalSlug.value = slug;
            addForm.action = '/admin/pages/update';
            formHeading.textContent = 'Edit Page';
