<?php
/**
 * Great Endured Technology — Admin Blog Post Editor View
...
<div style="padding: 30px 40px; background: #030508; min-height: 100vh;">
    ...
    <div style="margin-bottom: 25px;">
        <a href="/admin/posts" style="color: var(--color-cyan-pulse); text-decoration: none; font-size: 0.9rem; display: inline-flex; align-items: center; gap: 6px; font-weight: 500;">
            ← Back to Articles List
        </a>
    </div>

    <form id="post-editor-form" action="<?php echo $isEdit ? '/admin/posts/update' : '/admin/posts/create'; ?>" method="POST">
        <input type="hidden" name="csrf_token" value="<?php echo csrf_token(); ?>">
        <input type="hidden" name="original_slug" value="<?php echo $isEdit ? htmlspecialchars($post['slug']) : ''; ?>">
            ...
            <div>
                ...
                <div class="card" style="padding: 25px; border-radius: 12px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="slug" id="post-slug" class="form-input" 
                               placeholder="e.g. speed-optimizations" 
                               value="<?php echo $isEdit ? htmlspecialchars($post['slug']) : ''; ?>" required>
                        <p style="font-size: 0.7rem; color: var(--color-mist); margin-top: 5px;">Access path: /blog/<span id="slug-preview"><?php echo $isEdit ? htmlspecialchars($post['slug']) : 'speed-optimizations'; ?></span></p>
                    </div>

                    <div class="form-group" style="margin-bottom: 0;">
                        <label class="form-label">Meta Description</label>
                        <textarea name="meta_desc" class="form-input" style="min-height: 80px;" placeholder="Search snippet preview text (150-160 characters)..."><?php echo $isEdit ? htmlspecialchars($post['meta_desc']) : ''; ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="card" style="padding: 20px; border-radius: 12px; margin-bottom: 25px; background: #0d1117;">
                    ...
                    <div style="font-size: 0.85rem; color: var(--color-mist); display: flex; flex-direction: column; gap: 8px; margin-bottom: 20px;">
                        <div>Status: <strong style="color: var(--color-cyan-pulse);"><?php echo $isEdit ? 'Published' : 'New Draft'; ?></strong></div>
                        <div>Visibility: <strong style="color: #fff;">Public</strong></div>
                        <div>Format: <strong style="color: #fff;">Standard Blog Post</strong></div>
                    </div>

                    <div class="form-alert form-alert-success" id="post-success" style="display: none; padding: 10px; font-size: 0.8rem; margin-bottom: 15px;"></div>
                    <div class="form-alert form-alert-error" id="post-error" style="display: none; padding: 10px; font-size: 0.8rem; margin-bottom: 15px;"></div>

                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <button type="submit" class="btn btn-primary btn-full" id="post-submit-btn">
                            <?php echo $isEdit ? 'Update Article' : 'Publish Article'; ?>
                        </button>
                        <a href="/admin/posts" class="btn btn-secondary btn-full" style="border-color: rgba(255,255,255,0.05); text-align: center; display: block; line-height: 20px;">
                            Exit Workspace
                        </a>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const successAlert = document.getElementById('post-success');
    const errorAlert = document.getElementById('post-error');
    ...
    // Auto-generate slug from title (on creating a new post only)
    <?php if (!$isEdit): ?>
    titleInput.addEventListener('input', () => {
        ...
        slugPreview.textContent = slug || 'speed-optimizations';
    });
    <?php endif; ?>
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            submitBtn.textContent = "<?php echo $isEdit ? 'Update Article' : 'Publish Article'; ?>";
        }
    });
});
</script>
