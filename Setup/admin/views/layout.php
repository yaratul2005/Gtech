...
<body>
    ...
    (function() {
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            // Also trigger on form submits (like buttons click) to show loading
            document.querySelectorAll('form').forEach(form => {
                form.addEventListener('submit', () => {
                    const overlay = document.getElementById('fluid-wipe-overlay');
                    if (overlay && !form.getAttribute('action')?.includes('delete') && !form.getAttribute('action')?.includes('update')) {
                        overlay.style.transition = 'none';
                        overlay.style.transform = 'translateY(-100%)';
                        overlay.offsetHeight;
                        overlay.style.transition = 'transform 0.6s cubic-bezier(0.85, 0, 0.15, 1)';
